...
public class MoveSlideCapsule : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        _worldVelocity += (UseGravity ? Physics.gravity : Velocity) * Time.deltaTime;

        var newPos = transform.position + _worldVelocity * Time.deltaTime;

        var halfHeight = _collider.height;
        var wasGrounded = IsGrounded;
        IsGrounded = false;

        if (Velocity.y <= 0)
        {
            var rayLength = Mathf.Max(halfHeight + LegHeight, _worldVelocity.y);
            RaycastHit hitInfo;
        ...
        //while (true)// <- Bug; can enter infinite loop
        {
